...
let chatHistory = [
  {
    role: "user",
    parts: [{ text: "Act as a friendly assistant of user who give answer in short and concise manner. User like answer thank . So you should give answer within 2 to 3 lines paragraphs. For each answer keep this in mind and then give reply in 2 to 3 line paragraphs .  ok?" }],
  },
  {
    role: "model",
...
async function generateAIMessage(ele, userMsg = "") {
  ...
  try {
    ...
    text = response.text();
    if (text == "") {
      chatHistory[chatHistory.length - 1]?.parts.push({ text: "Apologies, but the response exceeds the maximum token limit of 200 set by DBH devs." });
      text = "Apologies, but the response exceeds the maximum token limit of 200 set by DBH devs.";
    }
  } catch (err) {
    text = "Please check your API key or internet connection and re-enter the Gemini API key. Page will reload in 4 seconds.";
